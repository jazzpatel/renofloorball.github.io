...
wd.addAsyncMethod('configureLogging', function(done) {
  if (env.VERBOSE) {
...
global.midwayUrl = function(testSuite, cat, title) {
  ...
  if (typeof testSuite === 'object') {
    ...
    cat = opts.cat;
    title = opts.title, uuid = opts.uuid;
  } else {
    if (!title) {
      title = cat;
      cat = undefined;
  ...
  return env.MIDWAY_ROOT_URL + '/test-page' + '?p=' + encodeURIComponent(cleanTitle) + '&ts=' + encodeURIComponent(testSuite) + (cat ? '&c=' + encodeURIComponent(cat) : '') + (uuid ? '&uuid=' + encodeURIComponent(uuid) : '');
};
...
global.sauceJobTitle = function(title) {
  return (env.TRAVIS_JOB_NUMBER ? '[' + env.TRAVIS_JOB_NUMBER + '] ' : '') + title.replace(/\(.*\)/g, '').replace(/\@[\w\-]+/g, '').trim();
};
