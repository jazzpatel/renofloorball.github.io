...
(function(process) {
  ...
  env.DESIRED = process.env.DESIRED ? JSON.parse(process.env.DESIRED) : {browserName: env.BROWSER};
  if (env.BROWSER === 'multi') {
  ...
  if (env.SAUCE) {
    env.MIDWAY_ROOT_URL = "http://localhost:" + env.PROXY_PORT + '/' + env.EXPRESS_PORT;
  } else {
  ...
  if (env.MULTI) {
    ...
    env.ENV_DESC = '(' + (env.SAUCE ? 'sauce' : 'local') + ', ' + (env.DESIRED.browserName || 'default') + ')';
  }
})(require('process'));
