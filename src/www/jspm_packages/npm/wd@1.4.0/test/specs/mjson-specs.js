  ...
  describe("mjson tests", function() {
    ...
    describe("promise api", function() {
      ...
      before(function(done) {
        server.post('/session').reply(303, "OK", {'Location': '/session/1234'}).get('/session/1234').reply(200, {
          status: 0,
          sessionId: '1234',
          value: {}
        });
      ...
      describe("by ios uiautomation", function() {
        it("element methods should work", function(done) {
          ...
          browser.element('-ios uiautomation', 'random stuff').should.eventually.exist.elementByIosUIAutomation('random stuff').should.eventually.exist.elementByIosUIAutomationOrNull('random stuff').should.eventually.exist.elementByIosUIAutomationIfExists('random stuff').should.eventually.exist.hasElementByIosUIAutomation('random stuff').should.eventually.be.ok.nodeify(done);
        });
        ...
        it("wait methods should work", function(done) {
          ...
          browser.waitForElement('-ios uiautomation', 'random stuff').should.eventually.exist.waitForElementByIosUIAutomation('random stuff').should.eventually.exist.waitForElementsByIosUIAutomation('random stuff').should.eventually.exist.nodeify(done);
        });
      });
      describe("by android uiautomator", function() {
        it("element methods should work", function(done) {
          ...
          browser.element('-android uiautomator', 'random stuff').should.eventually.exist.elementByAndroidUIAutomator('random stuff').should.eventually.exist.elementByAndroidUIAutomatorOrNull('random stuff').should.eventually.exist.elementByAndroidUIAutomatorIfExists('random stuff').should.eventually.exist.hasElementByAndroidUIAutomator('random stuff').should.eventually.be.ok.nodeify(done);
        });
        it("elements methods should work", function(done) {
          nock.cleanAll();
          server.post('/session/1234/elements', {
        ...
        it("wait methods should work", function(done) {
          ...
          browser.waitForElement('-android uiautomator', 'random stuff').should.eventually.exist.waitForElementByAndroidUIAutomator('random stuff').should.eventually.exist.waitForElementsByAndroidUIAutomator('random stuff').should.eventually.exist.nodeify(done);
        });
      });
      describe("by accessibility id", function() {
        it("element methods should work", function(done) {
          ...
          browser.element('accessibility id', 'random stuff').should.eventually.exist.elementByAccessibilityId('random stuff').should.eventually.exist.elementByAccessibilityIdOrNull('random stuff').should.eventually.exist.elementByAccessibilityIdIfExists('random stuff').should.eventually.exist.hasElementByAccessibilityId('random stuff').should.eventually.be.ok.nodeify(done);
        });
        it("elements methods should work", function(done) {
          nock.cleanAll();
          server.post('/session/1234/elements', {
        ...
        it("wait methods should work", function(done) {
          ...
          browser.waitForElement('accessibility id', 'random stuff').should.eventually.exist.waitForElementByAccessibilityId('random stuff').should.eventually.exist.waitForElementsByAccessibilityId('random stuff').should.eventually.exist.nodeify(done);
        });
      });
      describe("actions", function() {
        it("touch actions should work", function(done) {
          browser.chain().then(function() {
            nock.cleanAll();
            server.post('/session/1234/element', {
              "using": "id",
              "value": "random"
            ...
            var el;
            return browser.elementById('random').then(function(_el) {
              el = _el;
            }).then(function() {
        ...
        it("multi actions should work", function(done) {
          ...
          var el;
          browser.elementById('random').then(function(_el) {
            el = _el;
          }).then(function() {
            nock.cleanAll();
            ...
            nock.cleanAll();
            server.post('/session/1234/touch/multi/perform', {"actions": [[{
                "action": "tap",
                "options": {
                  "x": 100,
                  "y": 200
      ...
      describe("device methods", function() {
        ...
        it("touchId", function(done) {
          nock.cleanAll();
          server.post('/session/1234/appium/simulator/touch_id', {match: true}).times(1).reply(200, {sessionId: '1234'});
          browser.touchId(true).nodeify(done);
        });
        it("toggleTouchIdEnrollment", function(done) {
          nock.cleanAll();
          server.post('/session/1234/appium/simulator/toggle_touch_id_enrollment').times(1).reply(200, {sessionId: '1234'});
          browser.toggleTouchIdEnrollment(true).nodeify(done);
        });
        ...
        describe("pressKeycode", function() {
          it("keycode only", function(done) {
            nock.cleanAll();
            server.post('/session/1234/appium/device/press_keycode', {keycode: 3}).reply(200, {
              status: 0,
              sessionId: '1234'
            });
            browser.pressKeycode(3).nodeify(done);
          });
          it("keycode + metastate", function(done) {
            nock.cleanAll();
            server.post('/session/1234/appium/device/press_keycode', {
              keycode: 3,
              metastate: "abcd"
            }).reply(200, {
              status: 0,
            ...
            browser.pressKeycode(3, "abcd").nodeify(done);
          });
        });
        describe("rotateDevice", function() {
          it("without element", function(done) {
            nock.cleanAll();
            server.post('/session/1234/appium/device/rotate', {
              x: 114,
              y: 198,
              duration: 5,
              radius: 3,
          ...
          it("with element", function(done) {
            ...
            browser.elementById('random').then(function(el) {
              return browser.rotateDevice(el, {
                x: 114,
                y: 198,
                duration: 5,
                radius: 3,
        ...
        it("getCurrentPackage", function(done) {
          nock.cleanAll();
          server.get('/session/1234/appium/device/current_package').times(2).reply(200, {
            status: 0,
            sessionId: '1234',
            value: 'org.fake.package'
          });
          browser.getCurrentDeviceActivity().should.become('org.fake.package').getCurrentActivity().should.become('org.fake.package').nodeify(done);
        });
        ...
        it("hideDeviceKeyboard, passing key", function(done) {
          nock.cleanAll();
          server.post('/session/1234/appium/device/hide_keyboard', {keyName: "Done"}).times(1).reply(200, {
            status: 0,
            sessionId: '1234'
          });
          server.post('/session/1234/appium/device/hide_keyboard').times(1).reply(200, {
            status: 0,
            sessionId: '1234'
          });
          server.post('/session/1234/appium/device/hide_keyboard', {strategy: 'tapOutside'}).times(1).reply(200, {
            status: 0,
            sessionId: '1234'
          });
          server.post('/session/1234/appium/device/hide_keyboard', {
            strategy: 'pressKey',
            key: 'Done'
          }).times(1).reply(200, {
            status: 0,
          ...
          browser.hideKeyboard().hideDeviceKeyboard("Done").hideDeviceKeyboard({strategy: 'tapOutside'}).hideDeviceKeyboard({
            strategy: 'pressKey',
            key: 'Done'
          }).nodeify(done);
        });
        it("pushFileToDevice", function(done) {
          ...
          nock.cleanAll();
          server.post('/session/1234/appium/device/push_file', {
            path: remotePath,
            data: base64Data
          }).times(2).reply(200, {
            status: 0,
          ...
          browser.pushFileToDevice(remotePath, base64Data).pushFile(remotePath, base64Data).nodeify(done);
        });
        it("pullFileFromDevice", function(done) {
          ...
          var base64Data = new Buffer(stringData).toString('base64');
          nock.cleanAll();
          server.post('/session/1234/appium/device/pull_file', {path: remotePath}).times(2).reply(200, {
            status: 0,
            sessionId: '1234',
            value: base64Data
          });
          browser.pullFileFromDevice(remotePath, base64Data).should.become(base64Data).pullFile(remotePath, base64Data).should.become(base64Data).nodeify(done);
        });
        it("pullFolderFromDevice", function(done) {
          ...
          var base64Data = new Buffer(stringData).toString('base64');
          nock.cleanAll();
          server.post('/session/1234/appium/device/pull_folder', {path: remotePath}).times(2).reply(200, {
            status: 0,
            sessionId: '1234',
            value: base64Data
          });
          browser.pullFolderFromDevice(remotePath, base64Data).should.become(base64Data).pullFolder(remotePath, base64Data).should.become(base64Data).nodeify(done);
        });
        ...
        it("endTestCoverageForApp", function(done) {
          ...
          var base64Data = new Buffer(stringData).toString('base64');
          nock.cleanAll();
          server.post('/session/1234/appium/app/end_test_coverage', {
            intent: intent,
            path: path
          }).times(3).reply(200, {
            status: 0,
          ...
          browser.endTestCoverageForApp(intent, path).endTestCoverage(intent, path).endCoverage(intent, path).should.become(base64Data).nodeify(done);
        });
        ...
        it("setImmediateValueInApp", function(done) {
          ...
          server.post('/session/1234/element', {
            ...
            sessionId: '1234',
            value: {ELEMENT: '0'}
          }).post('/session/1234/appium/element/0/value', {value: "12345"}).times(4).reply(200, {
            status: 0,
            sessionId: '1234'
          });
          browser.elementById("random").then(function(el) {
        ...
        it("getDeviceTime", function(done) {
          nock.cleanAll();
          server.get('/session/1234/appium/device/system_time').reply(200, {
            status: 0,
            sessionId: '1234'
          });
          browser.getDeviceTime().nodeify(done);
        });
      });
    });
