...
if (env.SAUCE) {
  describe('sauce-helpers ' + env.ENV_DESC, function() {
    ...
    before(function() {
      ...
      return browser.configureLogging().init(desired).get('http://admc.io/wd/test-pages/guinea-pig.html').title().should.eventually.include("WD");
    });
    after(function() {
      return browser.quit().then(function() {
        if (!markPassed) {
          return (browser.sauceJobStatus(markPassed));
        }
      });
    });
