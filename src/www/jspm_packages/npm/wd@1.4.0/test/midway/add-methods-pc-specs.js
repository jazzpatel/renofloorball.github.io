...
describe('add-methods - promise-chain' + env.ENV_DESC, function() {
  ...
  var extraElementAsyncMethods = {textTwice: function(cb) {
      var _this = this;
      var result = '';
  ...
  var extraPromiseChainMethods = {
    sleepAndElementById: function(id) {
      return this.sleep(200).elementById(id);
    },
    sleepAndText: function(el) {
      return this.sleep(200).text(el);
    }
  };
  var extraElementPromiseChainMethods = {textTwice: function() {
      var _this = this;
      var result = '';
      return _this.text().then(function(text) {
        result += text;
        return _this;
  ...
  var extraPromiseNoChainMethods = {
    sleepAndElementById: function(id) {
      var _this = this;
      return this.sleep(200).then(function() {
  ...
  var extraElementPromiseNoChainMethods = {textTwice: function() {
      var _this = this;
      var result = '';
      return _this.text().then(function(text) {
        result += text;
  ...
  var allExtraMethodNames = _.union(_(extraAsyncMethods).keys().value(), _(extraPromiseChainMethods).keys().value(), _(extraPromiseNoChainMethods).keys().value());
  ...
  partials['wd.addPromisedMethod (chain)'] = '<div id="theDiv">Hello World!</div>';
  it('wd.addPromisedMethod (chain)', function() {
    _(extraPromiseChainMethods).each(function(method, name) {
      wd.addPromiseChainMethod(name, method);
    });
    return browser.sleepAndElementById('theDiv').should.be.fulfilled.sleepAndText().should.be.fulfilled.sleepAndElementById('theDiv').sleepAndText().should.eventually.include("Hello World!");
  });
  partials['wd.addElementPromisedMethod (chain)'] = '<div id="theDiv">\n' + '  <div id="div1">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '  </div>\n' + '  <div id="div2">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '    <span>three</span>\n' + '  </div>\n' + '</div>\n';
  it('wd.addElementPromisedMethod (chain)', function() {
    ...
    return browser.elementById('div1').textTwice().should.become('one twoone two');
  });
  partials['wd.addPromisedMethod (no-chain)'] = '<div id="theDiv">Hello World!</div>';
  ...
  partials['wd.addElementPromisedMethod (no-chain)'] = '<div id="theDiv">\n' + '  <div id="div1">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '  </div>\n' + '  <div id="div2">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '    <span>three</span>\n' + '  </div>\n' + '</div>\n';
  ...
  partials['wd.addAsyncMethod'] = '<div id="theDiv">Hello World!</div>';
  it('wd.addAsyncMethod', function() {
    _(extraAsyncMethods).each(function(method, name) {
      wd.addAsyncMethod(name, method);
    });
    return browser.elementByCssWhenReady('#theDiv', 500).text().should.become("Hello World!");
  });
  partials['wd.addElementAsyncMethod'] = '<div id="theDiv">\n' + '  <div id="div1">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '  </div>\n' + '  <div id="div2">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '    <span>three</span>\n' + '  </div>\n' + '</div>\n';
