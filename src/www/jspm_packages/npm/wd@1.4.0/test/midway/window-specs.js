...
describe('window ' + env.ENV_DESC, skip('explorer'), function() {
  ...
  afterEach(function() {
    return browser.windowHandles().then(function(handles) {
      ...
      _(handles).each(function(handle, i) {
        if (i > 0) {
          seq.push(function() {
            return browser.window(handle).close();
          });
        }
      });
      if (handles.length > 0) {
  ...
  it('browser.windowName', function() {
    return browser.execute('window.name="window-1"').windowName().should.become('window-1');
  });
  partials['browser.windowHandle'] = "";
  it('browser.windowHandle', function() {
    return browser.windowHandle().should.eventually.have.length.above(0);
  });
  partials['browser.newWindow'] = "";
  it('browser.newWindow', function() {
    return browser.url(function(url) {
      return browser.newWindow(url.replace("window_num=1", "window_num=2"), 'window-2');
    });
  });
  partials['browser.window'] = "";
  it('browser.window', function() {
    return Q.all([browser.url(), browser.windowHandle()]).then(function(res) {
      var url = res[0];
      var handle1 = res[1];
      return browser.newWindow(url.replace("window_num=1", "window_num=2"), 'window-2').window('window-2').windowName().should.become('window-2').window(handle1).windowHandle().should.become(handle1);
    });
  });
  partials['browser.windowHandles'] = "";
  it('browser.windowHandles', function() {
    return browser.url().then(function(url) {
      return browser.newWindow(url.replace("window_num=1", "window_num=2"), 'window-2').windowHandles().should.eventually.have.length(2).window('window-2').close().windowHandles().should.eventually.have.length(1);
    });
  });
  partials['browser.getWindowSize'] = "";
  it('browser.getWindowSize', function() {
    return browser.getWindowSize().then(function(size) {
      size.width.should.exist;
      size.height.should.exist;
    }).windowHandle().then(function(handle) {
      return browser.getWindowSize(handle).then(function(size) {
        size.width.should.exist;
        size.height.should.exist;
      });
    });
  ...
  it('browser.setWindowSize', skip('chrome'), function() {
    return browser.getWindowSize().then(function(size) {
      return browser.setWindowSize(size.width - 10, size.height - 5).getWindowSize().then(function(newSize) {
        ...
        return browser.setWindowSize(size.width - 15, size.height - 10, handle).getWindowSize().then(function(newSize) {
  ...
  it('browser.getWindowPosition', function() {
    return browser.getWindowPosition().then(function(pos) {
      pos.x.should.exist;
      pos.y.should.exist;
    }).windowHandle().then(function(handle) {
      return browser.getWindowPosition(handle).then(function(pos) {
        pos.x.should.exist;
        pos.y.should.exist;
      });
    });
  ...
  it('browser.setWindowPosition', function() {
    return browser.getWindowPosition().then(function(pos) {
      return browser.windowHandle(function(handle) {
        return browser.setWindowPosition(pos.width - 15, pos.height - 10, handle).getWindowPosition().then(function(newPos) {
