...
describe('config-http ' + env.ENV_DESC, function() {
  ...
  it('browser.configureHttp (using promise)', function() {
    ...
    return browser.configureHttp(newConfig).then(function() {
      browser._httpConfig.should.deep.equal(newConfig);
      wd.getHttpConfig().should.deep.equal(wdCurrent);
  ...
  it("setting global baseUrl", function() {
    ...
    promiseChainRemote();
    return browser.init(buildDesired(this.runnable().parent.title + " #1")).get(url).title().should.eventually.include('WD Tests - config-http').get(relUrl).title().should.eventually.include('WD Tests - config-http');
    ;
  });
  it("setting browser baseUrl", function() {
    ...
    return browser.init(buildDesired(this.runnable().parent.title + " #2")).then(function() {
      return browser.get(relUrl).should.eventually.include('WD Tests - config-http').should.be.rejected;
    }).configureHttp({baseUrl: baseUrl}).get(relUrl).title().should.eventually.include('WD Tests - config-http').get(url).title().should.eventually.include('WD Tests - config-http');
  });
  it("wd baseUrl override", function() {
    ...
    return browser.init(buildDesired(this.runnable().parent.title + " #3")).then(function() {
      return browser.get(relUrl).should.eventually.include('WD Tests - config-http').should.be.rejected;
    }).configureHttp({baseUrl: baseUrl}).get(relUrl).title().should.eventually.include('WD Tests - config-http').get(url).title().should.eventually.include('WD Tests - config-http');
  });
});
