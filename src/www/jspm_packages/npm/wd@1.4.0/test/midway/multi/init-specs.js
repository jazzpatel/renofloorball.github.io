...
describe('init ' + env.ENV_DESC, function() {
  ...
  it("default should be firefox", function() {
    ...
    return browser.init(buildDesired(this.runnable().parent.title + " #1", undefined, env.DESIRED.platform)).sessionCapabilities().should.eventually.have.property('browserName', 'firefox');
  });
  it("setting browser default", function() {
    ...
    return browser.init(buildDesired(this.runnable().parent.title + " #2", undefined, env.DESIRED.platform)).sessionCapabilities().should.eventually.have.property('browserName', 'chrome');
  });
  it("desired browser as parameter", function() {
    browser.defaultCapabilities.browserName = 'firefox';
    return browser.init(buildDesired(this.runnable().parent.title + " #3", 'chrome', env.DESIRED.platform)).sessionCapabilities().should.eventually.have.property('browserName', 'chrome');
  });
  if (env.SAUCE) {
    it("setting browser platform to VISTA", function() {
      browser.defaultCapabilities.platform = 'VISTA';
      browser.defaultCapabilities.browserName = 'firefox';
      return browser.init(buildDesired(this.runnable().parent.title + " #4")).sessionCapabilities().then(function(caps) {
    ...
    it("setting browser platform to LINUX", function() {
      ...
      return browser.init(buildDesired(this.runnable().parent.title + " #5")).sessionCapabilities().then(function(caps) {
        caps.platform.toLowerCase().should.equal('linux');
      });
    });
    it("configuring explorer in desired @saucelabs", function() {
      return browser.init(buildDesired(this.runnable().parent.title + " #6", 'iexplore', 'Windows 2008')).sessionCapabilities().then(function(sessionCapabilities) {
