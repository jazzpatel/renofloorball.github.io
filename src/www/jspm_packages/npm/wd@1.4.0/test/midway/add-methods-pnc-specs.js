...
describe('add-methods - promise-no-chain' + env.ENV_DESC, function() {
  ...
  var allExtraMethodNames = _.union(_(extraAsyncMethods).keys().value(), _(extraPromiseNoChainMethods).keys().value());
  ...
  partials['wd.addPromisedMethod'] = '<div id="theDiv">Hello World!</div>';
  it('wd.addPromisedMethod', function() {
    _(extraPromiseNoChainMethods).each(function(method, name) {
      wd.addPromiseMethod(name, method);
    });
    return browser.sleepAndElementById('theDiv').should.be.fulfilled.then(function() {
  ...
  partials['wd.addElementPromisedMethod'] = '<div id="theDiv">\n' + '  <div id="div1">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '  </div>\n' + '  <div id="div2">\n' + '    <span>one</span>\n' + '    <span>two</span>\n' + '    <span>three</span>\n' + '  </div>\n' + '</div>\n';
  it('wd.addElementPromisedMethod', function() {
    ...
    return browser.elementById('div1').then(function(el) {
      return el.textTwice();
