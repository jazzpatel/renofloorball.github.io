...
describe('ext-promises ' + env.ENV_DESC, function() {
  ...
  var extraMethods = {
    ...
    altSleepAndOk: function(ms) {
      return this.altSleep(ms).then(function() {
        return 'OK';
      });
    },
    altSleepAndElementById: function(ms, id) {
      return this.altSleep(ms).elementById(id);
    },
    elementByIdAndAltSleep: function(ms, id) {
      var _this = this;
      return this.elementById(id).then(function(el) {
  ...
  function initAndGet(that, desc) {
    ...
    return browser.configureLogging().init(mergeDesired(env.DESIRED, env.SAUCE ? sauceExtra : null)).get(midwayUrl(that.runnable().parent.title, that.runnable().title));
  }
  afterEach(function() {
    var _this = this;
  ...
  partials['addPromisedMethod (alt promise)'] = '<div id="theDiv">Hello World!</div>';
  it('addPromisedMethod (alt promise)', function() {
    ...
    return initAndGet(this, '#1').then(function() {
      return browser.altSleep(100).should.be.fulfilled.altSleepAndOk(100).should.become('OK');
    });
  });
  partials['addPromisedMethod (mixed promise)'] = '<div id="theDiv">Hello World!</div>';
  it('addPromisedMethod (mixed promise)', function() {
    ...
    return initAndGet(this, '#2').then(function() {
      return browser.altSleepAndElementById(100, 'theDiv').should.be.fulfilled.altSleepAndElementById(100, 'theDiv').text().should.become("Hello World!").elementByIdAndAltSleep(100, 'theDiv').text().should.become("Hello World!");
    });
  });
  partials['browser.resolve (alt promise)'] = '<div id="theDiv">Hello World!</div>';
  it('browser.resolve (alt promise)', function() {
    browser = newPromiseChainRemote();
    return initAndGet(this, '#3').then(function() {
      return browser.resolve(extraMethods.altSleepAndOk()).should.become('OK').noop().should.be.fulfilled;
    });
  });
});
