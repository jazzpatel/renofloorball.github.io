...
(function(process) {
  ...
  function buildMochaOpts(opts) {
    ...
    mochaOpts.env = function() {
      ...
      if (opts.midway) {
        ...
        expressPort++;
        if (env.SAUCE) {
          env.MIDWAY_ROOT_URL = 'http://127.0.0.1:' + PROXY_PORT + '/' + env.EXPRESS_PORT;
        }
  ...
  gulp.task('lint', function() {
    return gulp.src(['lib/**/*.js']).pipe(jshint()).pipe(jshint.reporter(jshintStylish)).pipe(jshint.reporter('fail'));
  });
  gulp.task('test-unit', function() {
    var opts = buildMochaOpts({unit: true});
    var mocha = mochaStream(opts);
    return gulp.src('test/specs/**/*-specs.js', {read: false}).pipe(mocha).on('error', console.warn.bind(console));
  });
  gulp.task('test-midway-multi', function() {
    var opts = buildMochaOpts({
      midway: true,
      browser: 'multi'
    });
    var mocha = mochaStream(opts);
    return gulp.src('test/midway/multi/**/*-specs.js', {read: false}).pipe(mocha).on('error', console.warn.bind(console));
  });
  _(BROWSERS).each(function(browser) {
    gulp.task('test-midway-' + browser, function() {
      ...
      var mocha = mochaStream(opts);
      return gulp.src(['test/midway/**/*-specs.js', '!test/midway/multi/**'], {read: false}).pipe(mocha).on('error', console.warn.bind(console));
    });
    gulp.task('test-e2e-' + browser, function() {
      var opts = buildMochaOpts({browser: browser});
      var mocha = mochaStream(opts);
      return gulp.src('test/e2e/**/*-specs.js', {read: false}).pipe(mocha).on('error', console.warn.bind(console));
    });
  });
  _(MOBILE_BROWSERS).each(function(browser) {
    gulp.task('test-midway-' + browser, function() {
      ...
      var mocha = mochaStream(opts);
      return gulp.src(['test/midway/api-nav-specs.js', 'test/midway/api-el-specs.js', 'test/midway/api-exec-specs.js', 'test/midway/mobile-specs.js'], {read: false}).pipe(mocha).on('error', console.warn.bind(console));
    });
  });
  gulp.task('test-midway', function() {
    ...
    return runSequence('pre-midway', midwayTestTasks).finally(function() {
