...
Q.spawn(function*() {
  ...
  try {
    yield browser.init({browserName: 'chrome'}).get("http://admc.io/wd/test-pages/guinea-pig.html").title().should.become('WD Tests');
    yield browser.elementById('i am a link').click();
    yield browser.eval("window.location.href").should.eventually.include('guinea-pig2');
    yield browser.back();
    yield browser.elementByCss('#comments').type('Bonjour!').getValue().should.become('Bonjour!');
  } finally {
    yield browser.quit();
  }
});
