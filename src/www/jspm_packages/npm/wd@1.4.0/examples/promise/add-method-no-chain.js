...
wd.addPromiseMethod('elementByCssSelectorWhenReady', function(selector, timeout) {
  var browser = this;
  return browser.waitForElementByCssSelector(selector, timeout).then(function() {
...
browser.init({browserName: 'chrome'}).then(function() {
  ...
  return browser.elementByCss('#comments').then(function(el) {
  ...
  return el.type("Bonjour!").then(function() {
