...
(function(process) {
  ...
  var jsonDocs = [JSON.parse(fs.readFileSync('tmp/webdriver-dox.json').toString()), JSON.parse(fs.readFileSync('tmp/element-dox.json').toString()), JSON.parse(fs.readFileSync('tmp/commands-dox.json').toString()), JSON.parse(fs.readFileSync('tmp/element-commands-dox.json').toString()), JSON.parse(fs.readFileSync('tmp/main-dox.json').toString()), JSON.parse(fs.readFileSync('tmp/asserters-dox.json').toString())];
  var resMapping = [];
  _(jsonWireFull).each(function(jw_v, jw_k) {
    ...
    if ((mappingType === 'full') || ((mappingType === 'supported') && (current.wd_doc.length > 0)) || ((mappingType === 'unsupported') && (current.wd_doc.length === 0))) {
  ...
  _(jsonDocs).each(function(jsonDoc) {
    _(jsonDoc).each(function(wd_v) {
      if (_(wd_v.tags).filter(function(t) {
        return t.type === 'jsonWire' || t.type === 'asserter' || t.type === 'wd';
      }).size() === 0) {
        var current = {
          extra: true,
          wd_doc: []
        ...
        if ((mappingType === 'full') || (mappingType === 'supported')) {
          resMapping.push(current);
        }
      }
    });
