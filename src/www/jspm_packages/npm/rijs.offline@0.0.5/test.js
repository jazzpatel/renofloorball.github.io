...
var expect = require('chai').expect,
    ...
    offline = require('./dist/index').default;
describe('Offline', function() {
  ...
  it('should cache to localStorage', function(done) {
    ...
    ripple1.resource('foo', [1, 2, 3]).resource('bar', function() {
      "baz";
    });
    expect(localStorage.ripple).to.not.be.ok;
    time(1050, function() {
      expect(localStorage.ripple.replace(/[ |;]/g, '')).to.eql('[{"name":"foo","body":[1,2,3],"headers":{"content-type":"application/data"}},{"name":"bar","headers":{"content-type":"application/javascript"},"body":"function(){\\"baz\\"}"}]');
      done();
    });
  });
  it('should load from localStorage', function(done) {
    ...
    time(1050, function() {
      ...
      expect(str(ripple2.resources.bar.body).replace(/ /g, '')).to.be.eql(str(function() {
        "baz";
      }).replace(/ /g, ''));
