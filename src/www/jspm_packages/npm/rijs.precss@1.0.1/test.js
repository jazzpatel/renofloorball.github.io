...
var expect = require('chai').expect,
    ...
    precss = require('./dist/index').default,
...
describe('Scoped CSS', function() {
  ...
  it('should not append css twice outside shadow dom', function(done) {
    container.innerHTML = '<css-2 css="foo.css"></css-2>' + '<css-2 css="foo.css"></css-2>';
    var ripple = precss(components(fn(css(core())))),
        result;
    ripple('foo.css', '* { color: red }');
    ripple('css-2', function() {
      result = true;
    });
    ripple.draw();
    time(40, function() {
  ...
  it('should render component with css loaded with shadow', function(done) {
    ...
    var ripple = shadow(precss(components(fn(css(core()))))),
        ...
        expected = hasShadow ? '<style resource="foo.css">* { color: red }</style>' : '<style resource="foo.css">[css~="foo.css"] * { color: red }</style>',
        result,
        el = container.firstChild;
    ripple('foo.css', '* { color: red }');
    ripple('css-shadow', function() {
      result = true;
    });
    ripple.draw();
    time(40, function() {
