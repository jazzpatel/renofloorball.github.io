...
var core = require('rijs.core').default,
    ...
    back = require('./dist/index').default,
    ...
    socket = {
      emit: function(type, data) {
        return socket.emitted.push(data);
      },
      request: request
    },
...
describe('Backpressure', function() {
  describe('Server', function() {
    ...
    it('should transform correctly', function() {
      ...
      expect(ripple.to({name: 'bar'}, {deps: {foo: 1}})).to.be.eql(false);
      expect(ripple.to({name: 'foo'}, {deps: {foo: 1}})).to.be.eql({name: 'foo'});
  ...
  describe('Client', function() {
    ...
    it('should refresh cache on connect', function() {
      ...
      time((d) => expect(emitted).to.eql([['change', {
        name: 'foo',
        type: 'pull'
      }], ['change', {
        name: 'bar',
    ...
    it('should subscribe node to updates', function(done) {
      ...
      var ripple = back(sync(draw(data(core())))),
          ...
          pull = 0,
          responses = 0;
      ripple.pull('bar', foo).then((d) => expect(d).to.be.eql('bar')).then((d) => pull++);
      expect(attr('data')(foo)).to.be.eql('bar');
      ripple.pull('bar', foo).then((d) => expect(d).to.be.eql('bar')).then((d) => pull++);
      expect(attr('data')(foo)).to.be.eql('bar');
      ripple.pull('baz', foo).then((d) => expect(d).to.be.eql('baz')).then((d) => pull++);
...
function sioServer(o) {
  return {
    ...
    on: function(type, fn) {
      if (type === 'change')
        receive = fn;
      if (type === 'connection' && includes('change')(str(fn)))
        fn({on: noop});
      if (type === 'connection')
        connection = fn;
    },
