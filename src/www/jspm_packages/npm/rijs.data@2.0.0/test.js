...
var core = require('rijs.core').default,
    ...
    data = require('./dist/index').default,
...
describe('Data Type', function() {
  ...
  it('should not lose all listeners', function() {
    var ripple = data(core());
    ripple('foo', ['foo']).on('change', String).on('change.foo', Date).on('foo', Function).on('foo.bar', Boolean);
    ripple('foo', ['bar']);
  ...
  it('should not lose log and update it on overwrite', function() {
    var ripple = data(core()),
        changes = [];
    ripple.on('change', function(d, change) {
