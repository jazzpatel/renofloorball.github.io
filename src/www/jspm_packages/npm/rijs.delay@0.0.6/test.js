...
var expect = require('chai').expect,
    ...
    delay = require('./dist/index').default,
    container,
    el1,
    el2;
describe('Delay Render', function() {
  before(function() {
    container = !client ? document.body.firstElementChild : document.body.appendChild(document.createElement('div'));
  });
  ...
  it('should postpone rendering by specified time', function(done) {
    ...
    time(400, function() {
      expect(el2.innerHTML).to.eql('done');
    });
    time(500, done);
  });
  it('should not affect delay-less components', function(done) {
    var ripple = delay(components(fn(core())));
    ripple('no-delay', function() {
      this.innerHTML = 'done';
    });
    time(40, function() {
      expect(el1.innerHTML).to.eql('done');
    });
    time(100, done);
  });
  it('should work in nested custom elements', function(done) {
    ...
    var ripple = delay(components(fn(core())));
    time(10, function() {
      ripple('x-el', function() {
        once(this)('ye-delay[delay="200"]', 1);
      });
      ripple('ye-delay', function() {
        this.innerHTML = 'done';
      });
    ...
    time(100, function() {
      expect(container.innerHTML).to.eql('<x-el><ye-delay inert=""></ye-delay></x-el>');
    });
    time(300, function() {
      expect(container.innerHTML).to.eql('<x-el><ye-delay>done</ye-delay></x-el>');
    });
    time(400, done);
  });
});
