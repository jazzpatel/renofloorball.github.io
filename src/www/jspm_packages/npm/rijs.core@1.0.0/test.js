...
var header = require('utilise/header'),
    ...
    core = require('./dist/index').default;
describe('Core', function() {
  ...
  it('should support method chaining api', function() {
    var ripple = core();
    ripple.resource('foo', 'bar').resource('bar', 'foo');
    expect(ripple('foo')).to.eql('bar');
  ...
  it('should register promise - resource', function(done) {
    var ripple = core();
    ripple(Promise.resolve({
      name: 'foo',
      body: 'bar'
    })).then((d) => expect('foo' in ripple.resources).to.be.ok).then((d) => expect(ripple.resources.foo.body).to.be.eql('bar')).then((d) => done());
    expect('foo' in ripple.resources).to.not.be.ok;
  });
  it('should register promise - body 1', function(done) {
    var ripple = core();
    ripple('foo', Promise.resolve('bar')).then((d) => expect('foo' in ripple.resources).to.be.ok).then((d) => expect(ripple.resources.foo.body).to.be.eql('bar')).then((d) => done());
    expect('foo' in ripple.resources).to.not.be.ok;
  });
