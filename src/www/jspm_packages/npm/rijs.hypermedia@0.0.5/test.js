...
(function(Buffer, process) {
  var expect = require('chai').expect,
      ...
      hypermedia = require('./dist/index').default,
  ...
  describe('Hypermedia API', function() {
    ...
    it('should alias resource', function(done) {
      ...
      ripple('repo', {
        owner: 'pemrouz',
        repo: 'ripple'
      }, {link: 'github.repository_url'}).on('change', function(change) {
        var repo = change.value;
        if (!ripple('repo').value)
          return;
        if (!repo.id)
          return;
    ...
    it('should expand optional params in links', function(done) {
      ...
      ripple('repo.issues_url', {number: 1}).on('change', function(change) {
        var issue = change.value;
        if (!issue.id)
          return;
        if (!ripple('repo.issues_url'))
          return;
        expect('repo' in ripple.resources).to.be.ok;
    ...
    it('should expand multiple parameterised links in path', function(done) {
      ...
      ripple('issue', {
        ...
        repo: 'ripple',
        number: 1
      }, {link: 'github.repository_url.issues_url'}).on('change', function(change) {
        var issue = change.value;
        if (!ripple('issue').value)
          return;
        if (!issue.id)
          return;
