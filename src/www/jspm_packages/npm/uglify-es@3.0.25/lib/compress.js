  ...
  (function() {
      ...
      function collapse(statements, compressor) {
              ...
              if (sym instanceof AST_Expansion) {
                ...
                if (all(elements, function(arg) {
                  ...
                  candidates.unshift(make_node(AST_VarDef, sym, {
                    name: sym.expression,
                    value: make_node(AST_Array, iife, {elements: elements})
                  }));
                }
              } else {
                var arg = iife.args[i];
                if (!arg)
                  arg = make_node(AST_Undefined, sym);
                else if (has_overlapping_symbol(fn, arg))
                  arg = null;
                if (arg)
                  candidates.unshift(make_node(AST_VarDef, sym, {
                    name: sym,
        ...
        function get_lhs(expr) {
          if (expr instanceof AST_VarDef && expr.name instanceof AST_SymbolDeclaration) {
            ...
            if (def.orig.length > 1 && !(expr.name instanceof AST_SymbolFunarg) || def.references.length == 1 && !compressor.exposed(def)) {
        ...
        function remove_candidate(expr) {
          if (expr.name instanceof AST_SymbolFunarg) {
            ...
            if (index < 0) {
              ...
              var args = iife.args;
              if (args[index])
                args[index] = make_node(AST_Number, args[index], {value: 0});
            }
            return true;
          }
          var found = false;
      ...
      function handle_if_return(statements, compressor) {
        ...
        for (var i = statements.length; --i >= 0; ) {
          ...
          if (stat instanceof AST_If && stat.body instanceof AST_Return) {
            ...
            if (multiple_if_returns && in_lambda && value && !stat.alternative && (!next || next instanceof AST_Return)) {
              CHANGED = true;
              stat = stat.clone();
            ...
            if (compressor.option("sequences") && in_lambda && !stat.alternative && prev instanceof AST_If && prev.body instanceof AST_Return && i + 2 == statements.length && next instanceof AST_SimpleStatement) {
              CHANGED = true;
              statements.push(make_node(AST_Return, next, {value: null}).transform(compressor));
              continue;
            }
          }
        }
    ...
    function extract_declarations_from_unreachable_code(compressor, stat, target) {
      ...
      stat.walk(new TreeWalker(function(node) {
        ...
        if (node instanceof AST_Defun && (node === stat || !compressor.has_directive("use strict"))) {
          target.push(node === stat ? node : make_node(AST_Var, node, {definitions: [make_node(AST_VarDef, node, {
    ...
    (function(def) {
      ...
      def(AST_ObjectProperty, function(compressor) {
        if (this.key instanceof AST_ObjectKeyVal && this.key.has_side_effects(compressor))
          return true;
    ...
    OPT(AST_BlockStatement, function(self, compressor) {
      ...
      switch (self.body.length) {
        case 1:
          if (!compressor.has_directive("use strict") && compressor.parent() instanceof AST_If || can_be_evicted_from_block(self.body[0])) {
            return self.body[0];
          }
          break;
        case 0:
    ...
    AST_Scope.DEFMETHOD("drop_unused", function(compressor) {
      ...
      var tw = new TreeWalker(function(node, descend) {
        if (node !== self) {
          ...
          if (assign_as_unused(node) instanceof AST_SymbolRef && scope === self && !is_ref_of(node.left, AST_SymbolBlockDeclaration)) {
      ...
      var tt = new TreeTransformer(function before(node, descend, in_list) {
        if (!compressor.option("keep_fnames") && node.name && (node instanceof AST_Function || node instanceof AST_ClassExpression)) {
        ...
        var parent = tt.parent();
        if (node instanceof AST_Definitions && !(parent instanceof AST_ForIn && parent.init === node) && (drop_vars || !(parent instanceof AST_Toplevel) && !(node instanceof AST_Var))) {
          var body = [],
              head = [],
              tail = [];
          var side_effects = [];
          node.definitions.forEach(function(def) {
            ...
            if (def.name instanceof AST_Destructuring)
              return tail.push(def);
            var sym = def.name.definition();
            if (!drop_vars && sym.global)
              return tail.push(def);
        ...
        if (drop_vars) {
          ...
          if (def instanceof AST_SymbolRef && !((def = def.definition()).id in in_use_ids) && (drop_vars || !def.global) && self.variables.get(def.name) === def) {
            if (node instanceof AST_Assign) {
    ...
    AST_Definitions.DEFMETHOD("to_assignments", function(compressor) {
      ...
      var assignments = this.definitions.reduce(function(a, def) {
        if (def.value && !(def.name instanceof AST_Destructuring)) {
          ...
          var var_ = make_node(AST_Var, def, {definitions: [varDef]});
          a.push(var_);
        }
        return a;
      }, []);
    ...
    OPT(AST_Call, function(self, compressor) {
      ...
      if (compressor.option("unused") && simple_args && (is_func_expr(fn) || compressor.option("reduce_vars") && fn instanceof AST_SymbolRef && is_func_expr(fn = fn.fixed_value())) && !fn.uses_arguments && !fn.uses_eval) {
        var pos = 0,
            last = 0;
        for (var i = 0,
            len = self.args.length; i < len; i++) {
          if (fn.argnames[i] instanceof AST_Expansion) {
            if (fn.argnames[i].expression.__unused)
              while (i < len) {
      ...
      if (compressor.option("unsafe_Func") && is_undeclared_ref(exp) && exp.name == "Function") {
        if (self.args.length == 0)
        ...
        if (all(self.args, function(x) {
          ...
          try {
            ...
            if (fun.body instanceof AST_Node) {
              fun.body = [make_node(AST_Return, fun.body, {value: fun.body})];
            }
            var code = OutputStream();
            AST_BlockStatement.prototype._codegen.call(fun, fun, code);
            self.args = [make_node(AST_String, self, {value: fun.argnames.map(function(arg) {
                return arg.print_to_string();
              }).join(",")}), make_node(AST_String, self.args[self.args.length - 1], {value: code.get().replace(/^\{|\}$/g, "")})];
            return self;
          } catch (ex) {
      ...
      if (stat instanceof AST_Node) {
        stat = make_node(AST_Return, stat, {value: stat});
      } else if (stat) {
        stat = stat[0];
      }
      ...
      if (is_func_expr(exp) && !exp.is_generator && !exp.async) {
        if (compressor.option("inline") && simple_args && !exp.name && !exp.uses_arguments && !exp.uses_eval && (exp.body instanceof AST_Node || exp.body.length == 1) && all(exp.argnames, function(arg) {
          if (arg instanceof AST_Expansion)
            return arg.expression.__unused;
          return arg.__unused;
      ...
      function trim_left_for_assignment() {
        for (var i = 0,
          ...
          while (true) {
            ...
            } else if (cdr instanceof AST_Call && !(left instanceof AST_PropAccess && cdr.expression.equivalent_to(left)) || cdr instanceof AST_PropAccess || cdr instanceof AST_Unary && !unary_side_effects(cdr.operator)) {
              field = "expression";
    ...
    OPT(AST_Binary, function(self, compressor) {
      ...
      if (compressor.option("comparisons"))
        switch (self.operator) {
          ...
          case "==":
          case "!=":
            if (compressor.option("typeofs") && self.left instanceof AST_String && self.left.value == "undefined" && self.right instanceof AST_UnaryPrefix && self.right.operator == "typeof") {
    ...
    OPT(AST_SymbolRef, function(self, compressor) {
      ...
      if (compressor.option("reduce_vars") && is_lhs(self, compressor.parent()) !== self) {
        ...
        if (compressor.option("unused") && is_func_expr(fixed) && d.references.length == 1 && !(d.scope.uses_arguments && d.orig[0] instanceof AST_SymbolFunarg) && !d.scope.uses_eval && compressor.find_parent(AST_Scope) === fixed.parent_scope) {
    ...
    AST_Lambda.DEFMETHOD("contains_this", function() {
      ...
      self.walk(new TreeWalker(function(node) {
        ...
        if (node instanceof AST_This)
          return result = true;
        if (node !== self && node instanceof AST_Scope && !(node instanceof AST_Arrow))
          return true;
      }));
      return result;
    });
    ...
    function literals_in_boolean_context(self, compressor) {
      if (compressor.option("booleans") && compressor.in_boolean_context()) {
        return best_of(compressor, self, make_sequence(self, [self, make_node(AST_True, self)]).optimize(compressor));
      }
      return self;
    }
    ;
    ...
    OPT(AST_Arrow, function(self, compressor) {
      if (!(self.body instanceof AST_Node))
        tighten_body(self.body, compressor);
      if (compressor.option("arrows") && self.body.length == 1 && self.body[0] instanceof AST_Return) {
    ...
    OPT(AST_Function, function(self, compressor) {
      tighten_body(self.body, compressor);
      if (compressor.option("arrows") && compressor.option("ecma") >= 6 && !self.name && !self.is_generator && !self.uses_arguments && !self.uses_eval) {
    ...
    OPT(AST_ConciseMethod, function(self, compressor) {
      if (compressor.option("arrows") && compressor.parent() instanceof AST_Object && self.value.body.length == 1 && self.value.body[0] instanceof AST_Return && self.value.body[0].value && !self.value.contains_this()) {
      ...
      if (compressor.option("ecma") >= 6) {
        ...
        if ((is_arrow_with_block || value instanceof AST_Function) && !value.name) {
          return make_node(AST_ConciseMethod, self, {
            ...
            key: key instanceof AST_Node ? key : make_node(AST_SymbolMethod, self, {name: key}),
