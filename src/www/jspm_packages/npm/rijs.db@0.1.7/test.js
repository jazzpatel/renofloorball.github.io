...
describe('Database', function() {
  ...
  it('should do nothing if fail to parse', function() {
    ...
    expect(ripple.connections).to.eql({wat: {
        id: 'wat',
  ...
  it('should do nothing if adaptor does not exist', function() {
    ...
    expect(ripple.connections).to.eql({type: {
        id: 'type',
  ...
  it('should update to db', function(done) {
    ...
    time(40, function() {
      expect(result).to.be.eql(['update', {
        foo: 'foo',
        id: 1
      }]);
    });
  ...
  function change(ripple) {
    return function(type) {
      return function(res, change) {
        if (type == 'add')
          dbadd(ripple)(res, change.key, change.value);
        if (type == 'update')
          dbupdate(ripple)(res, change.key, change.value);
        if (type == 'remove')
          dbremove(ripple)(res, change.key, change.value);
      };
    };
  }
  function dbload(ripple) {
    return function(res) {
      time(0, function() {
        result = ['load', ripple({
          name: res.name,
          body: mockdb[res.name],
          headers: {db: {loaded: true}}
        })];
      });
    };
  }
  ...
  function dbupdate(ripple) {
    return function(res, key, value) {
      time(0, function() {
        if (!key && !deep('headers.db.loaded')(res))
          return dbload(ripple)(res);
        if (key)
          result = ['update', res.body[key.split('.').shift()]];
      });
    };
  }
