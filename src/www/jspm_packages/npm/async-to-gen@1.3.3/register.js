...
function shouldTransform(filename, options) {
  ...
  var excludes = options && 'excludes' in options ? regexpPattern(options.excludes) : options && 'exclude' in options ? regexpPattern(options.exclude) : /\/node_modules\//;
...
function regexpPattern(pattern) {
  ...
  throw new Error('async-to-gen: includes and excludes must be RegExp or path strings. Got: ' + pattern);
}
