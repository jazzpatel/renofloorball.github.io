...
(function(process) {
  ...
  function asyncToGen(source, options) {
    ...
    editor.isEdited = Boolean(ast.containsAsync || ast.containsAsyncGen || ast.containsForAwaitOf);
  ...
  var asyncHelper = 'function __async(g){' + 'return new Promise(function(s,j){' + 'function c(a,x){' + 'try{' + 'var r=g[x?"throw":"next"](a)' + '}catch(e){' + 'j(e);' + 'return' + '}' + 'r.done?' + 's(r.value):' + 'Promise.resolve(r.value).then(c,d)' + '}' + 'function d(e){' + 'c(e,1)' + '}' + 'c()' + '})' + '}';
  module.exports.asyncHelper = asyncHelper;
  var asyncGenHelper = 'function __asyncGen(g){' + 'var q=[],' + 'T=["next","throw","return"],' + 'I={};' + 'for(var i=0;i<3;i++){' + 'I[T[i]]=a.bind(0,i)' + '}' + 'I[Symbol?' + 'Symbol.asyncIterator||(Symbol.asyncIterator=Symbol()):' + '"@@asyncIterator"]=function (){' + 'return this' + '};' + 'function a(t,v){' + 'return new Promise(function(s,j){' + 'q.push([s,j,v,t]);' + 'q.length===1&&c(v,t)' + '})' + '}' + 'function c(v,t){' + 'try{' + 'var r=g[T[t|0]](v),' + 'w=r.value&&r.value.__await;' + 'w?' + 'Promise.resolve(w).then(c,d):' + 'n(r,0)' + '}catch(e){' + 'n(e,1)' + '}' + '}' + 'function d(e){' + 'c(e,1)' + '}' + 'function n(r,s){' + 'q.shift()[s](r);' + 'q.length&&c(q[0][2],q[0][3])' + '}' + 'return I' + '}';
  module.exports.asyncGenHelper = asyncGenHelper;
  var asyncIteratorHelper = 'function __asyncIterator(o){' + 'var i=o[Symbol&&Symbol.asyncIterator||"@@asyncIterator"]||' + 'o[Symbol&&Symbol.iterator||"@@iterator"];' + 'if(!i)throw new TypeError("Object is not AsyncIterable.");' + 'return i.call(o)' + '}';
  module.exports.asyncIteratorHelper = asyncIteratorHelper;
  var asyncToGenVisitor = {
    AwaitExpression: {leave: leaveAwait},
    ...
    ThisExpression: {enter: function(editor, node, ast) {
        var envRecord = currentScope(ast);
        if (envRecord.async) {
          envRecord.referencesThis = true;
        }
      }},
    Identifier: {enter: function(editor, node, ast) {
    ...
    MemberExpression: {leave: leaveMemberExpression},
    AssignmentExpression: {leave: leaveAssignmentExpression},
    ForAwaitStatement: {leave: leaveForAwait}
  };
  function leaveAwait(editor, node, ast, stack) {
    var start;
    var end;
    var parentType = stack.parent.type;
    if (parentType === 'LogicalExpression' || parentType === 'BinaryExpression' || parentType === 'UnaryExpression' || parentType === 'ConditionalExpression' && node === stack.parent.test) {
      start = '(yield';
      end = ')';
    } else {
      start = 'yield';
  ...
  function createAsyncWrapping(node) {
    ...
    return [helperName + '(function*(' + argNames.join(',') + '){', (node.referencesThis ? '}.call(' : '}(') + argValues.join(',') + '))'];
  }
  function leaveMemberExpression(editor, node, ast, stack) {
    if (node.object.type !== 'Super')
      return;
    var contextNode = stack.parent;
    if (contextNode.operator === 'delete' || contextNode.type === 'AssignmentExpression' && contextNode.left === node)
      return;
    var envRecord = currentScope(ast);
    if (!envRecord.async)
      return;
  ...
  function leaveForAwait(editor, node, ast) {
    ...
    var tail = '}catch(e){' + error + '=e' + '}finally{' + 'try{' + '!' + step + '.done&&' + iter + '.return&&(' + toYield(iter + '.return()', ast) + ')' + '}finally{' + 'if(' + error + ')throw ' + error + '}' + '}';
