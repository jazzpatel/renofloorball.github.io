...
var fs = require('fs'),
    ...
    browser = require('./package.json!systemjs-json').browser,
    alias = "module.exports = require('{mod}')",
    entry = "require('./owner').{key} = require('{mod}')",
    modules = keys(require('./package.json!systemjs-json').devDependencies).filter(includes('utilise')).map(replace('utilise.', ''));
fs.readdirSync(__dirname).filter(includes('js')).filter(not(is.in(core))).map(prepend(__dirname + '/')).map(log('deleting')).map(fs.unlinkSync);
modules.map(append('.js')).map(log('creating')).map(args(0)(fs.createWriteStream)).map(function(s) {
...
modules.map(function(key) {
  return entry.replace('{key}', key).replace('{mod}', './' + key + '.js');
}).map(append('\n')).map(args(0)(index.write, index));
var tmp = fs.createWriteStream('tmp.js');
modules.filter(not(is.in(browser))).map(function(key) {
  return entry.replace('{key}', key).replace('{mod}', './' + key + '.js');
