...
(function(process) {
  (function() {
    ...
    function isLiteral(d) {
      return typeof d == 'object' && !(d instanceof Array);
    }
    function isTruthy(d) {
      return !!d == true;
    }
      ...
      return function(fn, ctx) {
        return function() {
          var i = is.arr(indices) ? indices : [indices],
              a = to.arr(arguments).filter(function(d, x) {
                return is.in(i)(x);
              });
    ...
    function attr(name, value) {
      ...
      return !is.str(name) && args == 2 ? attr(arguments[1]).call(this, arguments[0]) : !is.str(name) && args == 3 ? attr(arguments[1], arguments[2]).call(this, arguments[0]) : function(el) {
        var ctx = this || {};
        el = ctx.nodeName || is.fn(ctx.node) ? ctx : el;
        el = el.node ? el.node() : el;
        el = el.host || el;
        return args > 1 && value === false ? el.removeAttribute(name) : args > 1 ? (el.setAttribute(name, value), value) : el.attributes.getNamedItem(name) && el.attributes.getNamedItem(name).value;
      };
    }
    function wrap(d) {
      return function() {
    ...
    function str(d) {
      return d === 0 ? '0' : !d ? '' : is.fn(d) ? '' + d : is.obj(d) ? JSON.stringify(d) : String(d);
    }
    function key(k, v) {
      ...
      return function deep(o, i) {
        var masked = {};
        return !o ? undefined : !is.num(k) && !k ? o : is.arr(k) ? (k.map(copy), masked) : o[k] || !keys.length ? (set ? ((o[k] = is.fn(v) ? v(o[k], i) : v), o) : (is.fn(k) ? k(o) : o[k])) : (set ? (key(keys.join('.'), v)(o[root] ? o[root] : (o[root] = {})), o) : key(keys.join('.'))(o[root]));
        function copy(k) {
          var val = key(k)(o);
          if (val != undefined)
    ...
    function clone(d) {
      return !is.fn(d) && !is.str(d) ? parse(str(d)) : d;
    }
    function has(o, k) {
      return k in o;
    }
    ...
    function colorfill$1() {
      ['red', 'green', 'bold', 'grey', 'strip'].forEach(function(color) {
        !is.str(String.prototype[color]) && Object.defineProperty(String.prototype, color, {get: function() {
            return String(this);
          }});
      });
    }
    ...
    var owner = client ? window : global;
    ...
    function defaults(o, k, v) {
      if (o.host)
        o = o.host;
      return is.obj(k) ? (keys(k).map(function(i) {
        set(i, k[i]);
      }), o) : (set(k, v), o[k]);
      function set(k, v) {
        if (!is.def(o[k]))
    ...
    function el(selector) {
      var attrs = [],
          css = selector.replace(/\[(.+?)=(.*?)\]/g, function($1, $2, $3) {
            attrs.push([$2, $3]);
            return '';
          }).split('.'),
          tag = css.shift(),
    ...
    function emitterify(body) {
      ...
      function call(cb, pm) {
        cb.next ? cb.next(pm) : pm instanceof Array ? cb.apply(body, pm) : cb.call(body, pm);
      }
      function on(type, cb, once) {
        var id = type.split('.')[0],
            ns = type.split('.')[1],
            li = body.on[id] = body.on[id] || [];
        return !cb && ns ? (cb = body.on[id]['$' + ns]) ? cb : push(observable(body)) : !cb && !ns ? push(observable(body)) : cb && ns ? push((remove(li, body.on[id]['$' + ns] || -1), cb)) : cb && !ns ? push(cb) : false;
        function push(cb) {
          cb.once = once;
          cb.type = type;
          if (ns)
      ...
      function observable(parent, fn) {
        ...
        o.map = function(fn) {
          ...
          o.listeners[o.listeners.push(function(d, i) {
            n.next(fn(d, i, n));
          }) - 1].fn = fn;
          return n;
        };
        ...
        o.reduce = function(fn, seed) {
          var n = observable(o, fn);
          o.listeners[o.listeners.push(function(d, i) {
            n.next(seed = fn(seed, d, i, n));
          }) - 1].fn = fn;
          return n;
        };
        o.next = function(d) {
          o.resolve(d);
          o.listeners.map(function(fn) {
            fn(d, o.i);
          });
          o.i++;
          return o;
    ...
    function err(ns) {
      return function(d) {
        if (!owner.console || !console.error.apply)
          return d;
        is.arr(arguments[2]) && (arguments[2] = arguments[2].length);
        var args = to.arr(arguments),
    ...
    function form(root) {
      ...
      all('[name]', root).map(function(el) {
        var n = name(el),
            v = values[n] = el.state ? el.state.value : el.files ? el.files : el.type == 'checkbox' ? (values[n] || []).concat(el.checked ? el.value : []) : el.type == 'radio' ? (el.checked ? el.value : values[n]) : el.value;
        if (includes('is-invalid')(el.className))
          invalid.push(el);
      });
      return {
        values: values,
    ...
    function header(header, value) {
      ...
      return function(d) {
        return !d || !d.headers ? null : getter ? key(header)(d.headers) : key(header)(d.headers) == value;
      };
    }
    ...
    function once(nodes, enter, exit) {
      ...
      c.text = function(value) {
        ...
        return arguments.length === 0 ? n[0].textContent : (this.each(function(n, d, i) {
          ...
          if (this.textContent !== r)
            !(t = this.firstChild) ? this.appendChild(document.createTextNode(r)) : t.nodeName === '#text' ? t.nodeValue = r : this.textContent = r;
        }), this);
      };
      ...
      c.attr = function(key, value) {
        ...
        return arguments.length === 1 ? n[0].getAttribute(key) : (this.each(function(n, d, i) {
          var r = fn ? value.call(this, d, i) : value;
          if (!r && this.hasAttribute(key))
            this.removeAttribute(key);
          else if (r && this.getAttribute(key) !== r)
            this.setAttribute(key, r);
        }), this);
      };
      c.classed = function(key, value) {
        ...
        return arguments.length === 1 ? n[0].classList.contains(key) : (this.each(function(n, d, i) {
          var r = fn ? value.call(this, d, i) : value;
          if (r && !this.classList.contains(key))
            this.classList.add(key);
          else if (!r && this.classList.contains(key))
      ...
      c.property = function(key, value) {
        ...
        return arguments.length === 1 ? deep(key)(n[0]) : (this.each(function(n, d, i) {
          var r = fn ? value.call(this, d, i) : value;
          if (r !== undefined && deep(key)(this) !== r)
            deep(key, function() {
              return r;
            })(this);
        }), this);
      ...
      function c(s, d, k, b) {
        ...
        if (d === 1 && arguments.length == 2) {
          while (n[++p]) {
            ...
            if (j < 0)
              n[p].appendChild(tnodes[++t] = tenter[tenter.length] = create(selector, [n[p].__data__ || 1], 0));
            if ('function' === typeof tnodes[t].draw)
              tnodes[t].draw();
          }
        ...
        while (n[++p]) {
          ...
          while (n[p].children[++j]) {
            ...
            if ('function' === typeof n[p].children[j].draw)
              n[p].children[j].draw();
          }
          if (typeof selector === 'string') {
            ...
            while (++l < data.length) {
              (b ? n[p].insertBefore(tnodes[++t] = tenter[tenter.length] = n[p].templates[selector].cloneNode(false), n[p].querySelector(b)) : n[p].appendChild(tnodes[++t] = tenter[tenter.length] = n[p].templates[selector].cloneNode(false))).__data__ = data[l];
              if ('function' === typeof tnodes[t].draw)
                tnodes[t].draw();
            }
          } else {
    ...
    function event(node, index) {
      ...
      node.emit = function(event, detail) {
        node.dispatchEvent(event instanceof window.Event ? event : new window.CustomEvent(event, {
          detail: detail,
          bubbles: false,
          cancelable: true
        }));
      ...
      function reemit(event) {
        if ('object' === typeof window.d3)
          window.d3.event = event;
    ...
    function byKey(selector, data, key, b, parent, tnodes, tenter, texit) {
      ...
      while (parent.children[++c])
        if (!parent.children[c].matches(selector))
          continue;
        else
    ...
    function time(ms, fn) {
      return arguments.length === 1 ? setTimeout(ms) : setTimeout(fn, ms);
    }
    function unique(d, i) {
      if (!i)
        unique.matched = [];
      return !is.in(unique.matched)(d) ? unique.matched.push(d) : false;
    }
