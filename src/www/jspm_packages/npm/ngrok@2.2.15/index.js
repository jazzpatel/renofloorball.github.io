...
(function(process) {
  ...
  function connect(opts, cb) {
    ...
    lock('ngrok', function(release) {
      ...
      opts.authtoken ? authtoken(opts.authtoken, run, opts.configPath) : run(null);
    });
  }
  ...
  function runNgrok(opts, cb) {
    ...
    ngrok = spawn(bin, start, {cwd: __dirname + '/bin'});
  ...
  function _runTunnel(opts, cb) {
    ...
    var retry = function() {
      api.post({
        ...
        var notReady = resp.statusCode === 500 && /panic/.test(body) || resp.statusCode === 502 && body.details && body.details.err === 'tunnel session not ready yet';
        if (notReady) {
          return retries-- ? setTimeout(retry, 200) : cb(new Error(body));
        }
  ...
  function authtoken(token, cb, configPath) {
    ...
    var a = spawn(bin, authtoken, {cwd: __dirname + '/bin'});
  ...
  function disconnect(publicUrl, cb) {
    ...
    if (publicUrl) {
      return api.del(tunnels[publicUrl], function(err, resp, body) {
