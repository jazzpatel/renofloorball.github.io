...
(function(Buffer, process) {
  ...
  function extract(cb) {
    ...
    new Zip(localFile).extract({path: binPath}).once('error', error).once('extract', function() {
  ...
  function download(cb) {
    ...
    request.get(cdnFile).on('response', function(res) {
      ...
      readStream.pipe(fs.createWriteStream(localFile)).on('finish', () => extract(cb));
    }).on('error', function(e) {
